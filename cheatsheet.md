...
- `wf-review`: Wrapper around native `/review` (Claude/Codex) with verification, optional fix loop, and risk checks. (verify: `pnpm verify`)
...
### develop
- `bug-reproduction-validator`: Reproduce bugs and validate reports.
- `pr-comment-resolver`: Address PR comments and re-verify.
...
- `agent-native-reviewer`: Review features for agent-native parity (action + context).
- `architecture-strategist`: Review architecture decisions, constraints, tradeoffs.
- `code-simplicity-reviewer`: Simplify code; remove unnecessary complexity.
- `data-integrity-guardian`: Review data changes for integrity + safety.
- `data-migration-expert`: Validate ID mappings and migration correctness.
- `deployment-verification-agent`: Create go/no-go checklist for risky deploys.
- `kieran-python-reviewer`: Review Python code for strict conventions.
- `kieran-typescript-reviewer`: Review TypeScript code for strict conventions.
- `pattern-recognition-specialist`: Scan for patterns and anti-patterns.
- `performance-oracle`: Identify perf risks and optimization targets.
- `security-sentinel`: Security review for auth/data exposure.

### shape
- `spec-flow-analyzer`: Analyze specs for flow gaps.
...
- `best-practices-researcher`: Find external best practices and examples.
...
- `framework-docs-researcher`: Consult official framework docs for correct usage.
...
- `git-history-analyzer`: Analyze git history for context.
...
- `repo-research-analyst`: Survey repo structure and conventions.
